...
csrf_headers_names = [  # Headers qe mund te permbajne CSRF tokens / Headers that may contain CSRF tokens
    "X-XSRF-TOKEN",
    "X-CSRF-TOKEN",
    "CSRF-TOKEN",
    "XSRF-TOKEN",
...
csrf_params_names = [  # Parametrat qe mund te permbajne CSRF tokens / Parameters that may contain CSRF tokens
    "csrf",
    "xsrf",
    "token",
    "auth",
...
class BurpExtender(IBurpExtender, IProxyListener, ITab):
    ...
    def registerExtenderCallbacks(self, this_callbacks):  
        ...
        url = 'file:///C:/Users/Admin/Desktop/uni.jpg'
       
        self.home.add(JLabel(ImageIcon(URL(url))), BorderLayout.CENTER)
        self.home.doLayout()

        button = JButton('Start Scan', actionPerformed=run_scans)
        self.home.add(button, BorderLayout.SOUTH);        
        ...
        self.tabs.addTab('Home', self.home)
        self.tabs.addTab('CSRF', self.settings)
    ...
    def processProxyMessage(self, messageIsRequest, message):  ### IProxyListener
        ...
        if remove_csrf_headers:
            new_headers = self.filter_headers(headers)  # Largo CSRF headers / Remove CSRF headers
        ...
        if change_ct_to_plain and request_info.getContentType() not in [IRequestInfo.CONTENT_TYPE_URL_ENCODED, IRequestInfo.CONTENT_TYPE_MULTIPART]:
            for i in range(len(new_headers)):
                if new_headers[i].lower().startswith('content-type'):  # Ndrysho Content-Type ne text/plain / Change CT to text/plain
                    new_headers[i] = 'Content-Type: text/plain'

        if remove_csrf_params:
            for parameter in parameters:  # Largo parametrat CSRF nga trupi i request-it ose URL / Remove CSRF parameters from request's body or URL
        ...
        if change_ct_to_json and request_info.getContentType() == IRequestInfo.CONTENT_TYPE_URL_ENCODED:
            for i in range(len(new_headers)):
                if new_headers[i].lower().startswith('content-type'):  # Ndrysho ne JSON nga URL-encoded / Change to JSON from URL-encoded
        ...
        new_request = helpers.buildHttpMessage(new_headers, body)  # Krijimi i nje request-i te ri me nje Content-Length valide / Creation of a new request with valid Content-Length
        ...
        if (change_method_to_post and request_method != 'POST') or (change_to_get and not change_ct_to_json and \
            ...
            if change_to_get:
                new_request = helpers.toggleRequestMethod(new_request)   # Ndrysho cfaredo request-i URL-encoded ne GET / Change any URL-encoded request to GET
...
class SendToWhitelist(IContextMenuFactory):
    ...
    def createMenuItems(self, invocation):  
        ...
        add_to_whitelist_menu_item = JMenuItem('>> Add to CSRF whitelist <<')
